...
const Navbar = () => {
  ...
  const [isKirtanDropdownOpen, setIsKirtanDropdownOpen] = useState(false);
  ...
  const handleDropdownClick = (dropdown) => {
    ...
    else if (dropdown === 'kirtan')
      {
        setIsKirtanDropdownOpen(!isKirtanDropdownOpen)
        setIsBlogDropdownOpen(false);
        setisKathaDropdownOpen(false);
        setIsAboutDropdownOpen(false);
    }
    else if (dropdown === 'quotes')
      {
        setIsQuotesDropdownOpen(!isQuotesDropdownOpen);
        setIsKirtanDropdownOpen(false)
        setIsBlogDropdownOpen(false);
        setisKathaDropdownOpen(false);
        setIsAboutDropdownOpen(false);
    }
...
<div className="relative">
  ...
  >
    <span>Quotes</span>
    {isQuotesDropdownOpen ? <FaChevronUp className="ml-2" /> : <FaChevronDown className="ml-2" />}
  </button>
 ...
 */}
        ...
        <div className="relative">
        <button
          onClick={() => handleDropdownClick('kirtan')}
          className="px-4 py-2 text-gray-800 rounded-full hover:bg-gray-200 transition-colors duration-300 flex items-center space-x-2"
        >
          <span>Kirtan & Lectures</span>
          {isKirtanDropdownOpen ? <FaChevronUp className="ml-2" /> : <FaChevronDown className="ml-2" />}
        </button>
        {isKirtanDropdownOpen && (
          <div className="absolute left-0 mt-2 bg-white border border-gray-300 rounded-lg w-48 z-20">
            <Link
              to="/kirtan/reels"
              className="block px-4 py-2 text-gray-800 hover:bg-gray-200 transition-colors duration-300"
